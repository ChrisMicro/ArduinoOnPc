/*-------------------------------------------------------------------------
  ...
  Addapted for ArduinoOnPc by ChrisMicro, July 2017
...
#define NOPx7 { __builtin_arc_nop(); \
  ...
  {
	uint32_t val=  getPixelColor(n);
	uint8_t r=val>>16;
	uint8_t g=val>>8;
	uint8_t b=val;

	int numberOfLedsLineBreak=16;
	int xn=n;
	int y=pos_y;
	while( xn>=numberOfLedsLineBreak )
	{
